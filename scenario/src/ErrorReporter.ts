import {ComptrollerErr, BAIControllerErr, TokenErr} from './ErrorReporterConstants';
...
class BRTokenErrorReporterType implements ErrorReporter {
  getError(error: any): string | null {
    if (error === null) {
      return null;
    } else {
...
class BAIControllerErrorReporterType implements ErrorReporter {
  getError(error: any): string | null {
    if (error === null) {
      return null;
    } else {
      // TODO: This probably isn't right...
      return BAIControllerErr.ErrorInv[Number(error)];
    }
  }
  ...
  getInfo(info: any): string | null {
    if (info === null) {
      ...
      // TODO: This probably isn't right...
      return BAIControllerErr.FailureInfoInv[Number(info)];
    }
  }

  getDetail(error: any, detail: number): string {
    if (this.getError(error) === "REJECTION") {
      let baicontrollerError = BAIControllerErrorReporter.getError(detail);

      if (baicontrollerError) {
        return baicontrollerError;
      }
    }
...
export const BRTokenErrorReporter = new BRTokenErrorReporterType();
export const ComptrollerErrorReporter = new ComptrollerErrorReporterType();
export const BAIControllerErrorReporter = new BAIControllerErrorReporterType();
