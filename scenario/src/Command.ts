        ...
        if (eventArg === undefined) {
          ...
          try {
            ...
            if (arg.rescue) {
              ...
              // `Gate (BRToken Ckb Address) (... deploy brToken)`
              // could be used to deploy a brToken if it doesn't exist, but
              // since there is no BRToken, that check would raise (when we'd
