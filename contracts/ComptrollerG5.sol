...
contract ComptrollerG5 is ComptrollerV5Storage, ComptrollerInterfaceG2, ComptrollerErrorReporter, ExponentialNoError {
    ...
    function setBrainiacSpeedInternal(BRToken brToken, uint brainiacSpeed) internal {
        ...
        if (currentBrainiacSpeed != 0) {
            ...
            // Add the BRN market
            Market storage market = markets[address(brToken)];
            require(market.isListed == true, "brainiac market is not listed");
     ...
     */
    function distributeBAIMinterBrainiac(address baiMinter) public {
        if (address(baiVaultAddress) != address(0)) {
            releaseToVault();
        }
        ...
        if (address(baiController) != address(0)) {
            ...
            if (err == uint(Error.NO_ERROR)) {
                brainiacAccrued[baiMinter] = baiMinterAccrued;
    ...
    function claimBrainiac(address[] memory holders, BRToken[] memory brTokens, bool borrowers, bool suppliers) public {
        ...
        for (j = 0; j < holders.length; j++) {
            distributeBAIMinterBrainiac(holders[j]);
            brainiacAccrued[holders[j]] = grantBRNInternal(holders[j], brainiacAccrued[holders[j]]);
        }
    ...
    /**
     * @notice Transfer BRN to the user
     ...
     */
    function grantBRNInternal(address user, uint amount) internal returns (uint) {
     ...
     * @param amount The amount of BRN to (possibly) transfer
     */
    function _grantBRN(address recipient, uint amount) public {
        require(adminOrInitializing(), "only admin can grant brn");
        uint amountLeft = grantBRNInternal(recipient, amount);
     ...
     */
    function _setBrainiacBAIRate(uint brainiacBAIRate_) public onlyAdmin {
     ...
     */
    function _setBrainiacBAIVaultRate(uint brainiacBAIVaultRate_) public onlyAdmin {
     ...
     */
    function _setBrainiacSpeed(BRToken brToken, uint brainiacSpeed) public {
        require(adminOrInitializing(), "only admin can set brainiac speed");
    ...
    /**
     * @notice Transfer BRN to BAI Vault
     */
    function releaseToVault() public {
        ...
        // releaseAmount = brainiacBAIVaultRate * deltaBlocks
        uint256 _releaseAmount = mul_(brainiacBAIVaultRate, deltaBlocks);

        if (_releaseAmount < minReleaseAmount) {
            return;
        }

        if (brnBalance >= _releaseAmount) {
            actualAmount = _releaseAmount;
        } else {
            actualAmount = brnBalance;
        }

        releaseStartBlock = getBlockNumber();

        brn.transfer(baiVaultAddress, actualAmount);
        emit DistributedBAIVaultBrainiac(actualAmount);
