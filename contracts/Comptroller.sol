...
contract Comptroller is ComptrollerV7Storage, ComptrollerInterfaceG2, ComptrollerErrorReporter, ExponentialNoError {
    ...
    event ActionPausedMarket(BRToken brToken, string action, bool pauseState);

    /// @notice Emitted when Brainiac BAI rate is changed
    event NewBrainiacBAIRate(uint oldBrainiacBAIRate, uint newBrainiacBAIRate);

    /// @notice Emitted when Brainiac BAI Vault rate is changed
    event NewBrainiacBAIVaultRate(uint oldBrainiacBAIVaultRate, uint newBrainiacBAIVaultRate);
    ...
    /// @notice Emitted when BRN is distributed to BAI Vault
    event DistributedBAIVaultBrainiac(uint amount);
    ...
    /// @notice Emitted when borrow cap for a brToken is changed
    event NewBorrowCap(BRToken indexed brToken, uint newBorrowCap);
    ...
    /// @notice Emitted when Brainiac is granted by admin
    event BrainiacGranted(address recipient, uint amount);
    ...
    function mintAllowed(address brToken, address minter, uint mintAmount) external onlyProtocolAllowed returns (uint) {
        ...
        distributeSupplierBrainiac(brToken, minter);
    ...
    function redeemAllowed(address brToken, address redeemer, uint redeemTokens) external onlyProtocolAllowed returns (uint) {
        ...
        // Keep the flywheel moving
        updateBrainiacSupplyIndex(brToken);
        distributeSupplierBrainiac(brToken, redeemer);
    ...
    function borrowAllowed(address brToken, address borrower, uint borrowAmount) external onlyProtocolAllowed returns (uint) {
        ...
        uint borrowCap = borrowCaps[brToken];
        // Borrow cap of 0 corresponds to unlimited borrowing
        if (borrowCap != 0) {
            uint totalBorrows = BRToken(brToken).totalBorrows();
        ...
        distributeBorrowerBrainiac(brToken, borrower, borrowIndex);
    ...
    function liquidateBorrowAllowed(
        ...
        if (!(markets[brTokenBorrowed].isListed || address(brTokenBorrowed) == address(baiController)) || !markets[brTokenCollateral].isListed) {
            return uint(Error.MARKET_NOT_LISTED);
        }
        ...
        uint borrowBalance;
        if (address(brTokenBorrowed) != address(baiController)) {
            borrowBalance = BRToken(brTokenBorrowed).borrowBalanceStored(borrower);
        } else {
    ...
    function seizeAllowed(
        ...
        if (!markets[brTokenCollateral].isListed || !(markets[brTokenBorrowed].isListed || address(brTokenBorrowed) == address(baiController))) {
            return uint(Error.MARKET_NOT_LISTED);
        }
        ...
        distributeSupplierBrainiac(brTokenCollateral, borrower);
        distributeSupplierBrainiac(brTokenCollateral, liquidator);
    ...
    function transferAllowed(address brToken, address src, address dst, uint transferTokens) external onlyProtocolAllowed returns (uint) {
        ...
        // Keep the flywheel moving
        updateBrainiacSupplyIndex(brToken);
        distributeSupplierBrainiac(brToken, src);
        distributeSupplierBrainiac(brToken, dst);
     ...
     * @return (errorCode, number of brTokenCollateral tokens to be seized in a liquidation)
     */
    function liquidateBAICalculateSeizeTokens(address brTokenCollateral, uint actualRepayAmount) external view returns (uint, uint) {
        (uint err, uint seizeTokens) = comptrollerLens.liquidateBAICalculateSeizeTokens(
            address(this), 
            brTokenCollateral, 
            actualRepayAmount);
        return (err, seizeTokens);
    ...
    function _setBRNTokenAddress(address _brn) external returns (uint) {
     ...
     function _setBrBRNTokenAddress(address _brBrn) external returns (uint) {
        // Check caller is admin
        ensureAdmin();
        brBrnToken = _brBrn;
        return uint(Error.NO_ERROR);
    ...
    function _setCollateralFactor(BRToken brToken, uint newCollateralFactorMantissa) external returns (uint) {
        ...
        ensureNonzeroAddress(address(brToken));
    ...
    function _supportMarket(BRToken brToken) external returns (uint) {
        ...
        // Note that isBrainiac is not in active use anymore
    ...
    function _setMintPaused(BRToken brToken, bool state) public onlyListedMarket(brToken) validPauseState(state) returns (bool) {
        ...
        emit ActionPausedMarket(brToken, "Mint", state);
        return state;
    }
    ...
    function _setBorrowPaused(BRToken brToken, bool state) public onlyListedMarket(brToken) validPauseState(state) returns (bool) {
        ...
        emit ActionPausedMarket(brToken, "Borrow", state);
        return state;
    }
    ...
    /**
      * @notice Set the given borrow caps for the given brToken markets. Borrowing that brings total borrows to or above borrow cap will revert.
      * @dev Admin or borrowCapGuardian function to set the borrow caps. A borrow cap of 0 corresponds to unlimited borrowing.
      * @param brTokens The addresses of the markets (tokens) to change the borrow caps for
      * @param newBorrowCaps The new borrow cap values in underlying to be set. A value of 0 corresponds to unlimited borrowing.
      */
    function _setMarketBorrowCaps(BRToken[] calldata brTokens, uint[] calldata newBorrowCaps) external {
        require(msg.sender == admin || msg.sender == borrowCapGuardian, "only admin or borrow cap guardian can set borrow caps");

        uint numMarkets = brTokens.length;
        uint numBorrowCaps = newBorrowCaps.length;

        require(numMarkets != 0 && numMarkets == numBorrowCaps, "invalid input");

        for(uint i = 0; i < numMarkets; i++) {
            borrowCaps[address(brTokens[i])] = newBorrowCaps[i];
            emit NewBorrowCap(brTokens[i], newBorrowCaps[i]);
        }
    }
    ...
    //     ensureNonzeroAddress(address(baiController_));

    //     BAIControllerInterface oldBaiController = baiController;
    //     baiController = baiController_;
    //     emit NewBAIController(oldBaiController, baiController_);
    ...
    function setBrainiacSpeedInternal(BRToken brToken, uint brainiacSpeed) internal {
        uint currentBrainiacSpeed = brainiacSpeeds[address(brToken)];
        if (currentBrainiacSpeed != 0) {
            // note that BRN speed could be set to 0 to halt liquidity rewards for a market
        ...
        } else if (brainiacSpeed != 0) {
            // Add the BRN market
            Market memory market = markets[address(brToken)];
            require(market.isListed, "brainiac market is not listed");

            if (brainiacSupplyState[address(brToken)].index == 0 && brainiacSupplyState[address(brToken)].block == 0) {
                brainiacSupplyState[address(brToken)] = BrainiacMarketState({
        ...
        if (brainiacBorrowState[address(brToken)].index == 0 && brainiacBorrowState[address(brToken)].block == 0) {
                brainiacBorrowState[address(brToken)] = BrainiacMarketState({
                    index: brainiacInitialIndex,
                    block: safe32(getBlockNumber(), "block number exceeds 32 bits")
                });
            }
        ...
        if (currentBrainiacSpeed != brainiacSpeed) {
            brainiacSpeeds[address(brToken)] = brainiacSpeed;
            emit BrainiacSpeedUpdated(brToken, brainiacSpeed);
        }
    }
     ...
     */
    function distributeSupplierBrainiac(address brToken, address supplier) internal {
      

        BrainiacMarketState memory supplyState = brainiacSupplyState[brToken];
        ...
        brainiacAccrued[supplier] = supplierAccrued;
     ...
     */
    function distributeBorrowerBrainiac(address brToken, address borrower, Exp memory marketBorrowIndex) internal {
       

        BrainiacMarketState memory borrowState = brainiacBorrowState[brToken];
        ...
        if (borrowerIndex.mantissa > 0) {
            ...
            brainiacAccrued[borrower] = borrowerAccrued;
     ...
     */
     function claimBrainiac(address[] memory holders, BRToken[] memory brTokens, bool borrowers, bool suppliers) public {
        claimBrainiac(holders, brTokens, borrowers, suppliers, false);
    }
     ...
     * @param borrowers Whether or not to claim BRN earned by borrowing
     * @param suppliers Whether or not to claim BRN earned by supplying
     * @param collateral Whether or not to use BRN earned as collateral, only takes effect when the holder has a shortfall
     */
    function claimBrainiac(address[] memory holders, BRToken[] memory brTokens, bool borrowers, bool suppliers, bool collateral) public {
        uint j;
        // Save shortfalls of all holders
        // if there is a positive shortfall, the BRN reward is accrued,
        // but won't be granted to this holder
        uint[] memory shortfalls = new uint[](holders.length);
        for (j = 0; j < holders.length; j++) {
            (, , uint shortfall) = getHypotheticalAccountLiquidityInternal(holders[j], BRToken(0), 0, 0);
        ...
        for (uint i = 0; i < brTokens.length; i++) {
            ...
            if (borrowers) {
                ...
                updateBrainiacBorrowIndex(address(brToken), borrowIndex);
                for (j = 0; j < holders.length; j++) {
                    distributeBorrowerBrainiac(address(brToken), holders[j], borrowIndex);
                    brainiacAccrued[holders[j]] = grantBRNInternal(holders[j], brainiacAccrued[holders[j]], shortfalls[j], collateral);
                }
            }
            if (suppliers) {
                updateBrainiacSupplyIndex(address(brToken));
                for (j = 0; j < holders.length; j++) {
                    distributeSupplierBrainiac(address(brToken), holders[j]);
                    brainiacAccrued[holders[j]] = grantBRNInternal(holders[j], brainiacAccrued[holders[j]], shortfalls[j], collateral);
                }
            }
        }
    ...
    /**
     * @notice Claim all the brn accrued by holder in all markets, a shorthand for `claimBrainiac` with collateral set to `true`
     * @param holder The address to claim BRN for
     */
    function claimBrainiacAsCollateral(address holder) external {
        address[] memory holders = new address[](1);
        holders[0] = holder;
        claimBrainiac(holders, allMarkets, true, true, true);
    }

    /**
     * @notice Transfer BRN to the user with user's shortfall considered
     ...
     * @param amount The amount of BRN to (possibly) transfer
     * @param shortfall The shortfall of the user
     * @param collateral Whether or not we will use user's brainiac reward as collateral to pay off the debt
     * @return The amount of BRN which was NOT transferred to the user
     */
    function grantBRNInternal(address user, uint amount, uint shortfall, bool collateral) internal returns (uint) {
        BRN brn = BRN(getBRNAddress());
        uint brainiacRemaining = brn.balanceOf(address(this));
        bool bankrupt = shortfall > 0;

        if (amount == 0 || amount > brainiacRemaining) {
            return amount;
        }
        ...
        if (!bankrupt) {
            brn.transfer(user, amount);
            return 0;
        }
        // If user's bankrupt and doesn't use pending brn as collateral, don't grant
        // anything, otherwise, we will transfer the pending brn as collateral to 
        // brBRN token and mint brBRN for the user.
        // 
        // If mintBehalf failed, don't grant any brn
        require(collateral, "bankrupt accounts can only collateralize their pending brn rewards");

        brn.approve(getBRNBRTokenAddress(), amount);
        require(
            BRErc20Interface(getBRNBRTokenAddress()).mintBehalf(user, amount) == uint(Error.NO_ERROR),
            "mint behalf error during collateralize brn"
        );

        // set brainiacAccrue[user] to 0
        return 0;
    }
    ...
    /**
     * @notice Transfer BRN to the recipient
     * @dev Note: If there is not enough BRN, we do not perform the transfer all.
     * @param recipient The address of the recipient to transfer BRN to
     * @param amount The amount of BRN to (possibly) transfer
     */
    function _grantBRN(address recipient, uint amount) external {
        require(adminOrInitializing(), "only admin or impl can grant brn");
        uint amountLeft = grantBRNInternal(recipient, amount, 0, false);
        require(amountLeft == 0, "insufficient brn for grant");
        emit BrainiacGranted(recipient, amount);
    }

    /**
     * @notice Set the amount of BRN distributed per block to BAI Vault
     * @param brainiacBAIVaultRate_ The amount of BRN wei per block to distribute to BAI Vault
     */
    // function _setBrainiacBAIVaultRate(uint brainiacBAIVaultRate_) external {
    //     ensureAdmin();

    //     uint oldBrainiacBAIVaultRate = brainiacBAIVaultRate;
    //     brainiacBAIVaultRate = brainiacBAIVaultRate_;
    //     emit NewBrainiacBAIVaultRate(oldBrainiacBAIVaultRate, brainiacBAIVaultRate_);
    // }
     ...
     */
    // function _setBAIVaultInfo(address vault_, uint256 releaseStartBlock_, uint256 minReleaseAmount_) external {
    //     ensureAdmin();
    ...
    /**
     * @notice Set BRN speed for a single market
     * @param brToken The market whose BRN speed to update
     * @param brainiacSpeed New BRN speed for market
     */
    function _setBrainiacSpeed(BRToken brToken, uint brainiacSpeed) external {
        require(adminOrInitializing(), "only admin or impl can set brainiac speed");
        ensureNonzeroAddress(address(brToken));
        setBrainiacSpeedInternal(brToken, brainiacSpeed);
    }
    ...
    /**
     * @notice Return the address of the BRN brToken
     * @return The address of BRN brToken
     */
