...
import "../BRErc20Immutable.sol";
import "../BRErc20Delegator.sol";
import "../BRErc20Delegate.sol";
import "../VDaiDelegate.sol";
import "./ComptrollerScenario.sol";

contract BRErc20Harness is BRErc20Immutable {
    ...
    function harnessRedeemFresh(address payable account, uint brTokenAmount, uint underlyingAmount) public returns (uint) {
        return super.redeemFresh(account, brTokenAmount, underlyingAmount);
    }
    ...
    function harnessLiquidateBorrowFresh(address liquidator, address borrower, uint repayAmount, BRToken brTokenCollateral) public returns (uint) {
        (uint err,) = liquidateBorrowFresh(liquidator, borrower, repayAmount, brTokenCollateral);
        return err;
    }
...
contract BRErc20Scenario is BRErc20Immutable {
    constructor(address underlying_,
                ComptrollerInterface comptroller_,
                InterestRateModel interestRateModel_,
                uint initialExchangeRateMantissa_,
...
contract VEvil is BRErc20Scenario {
    constructor(address underlying_,
                ComptrollerInterface comptroller_,
                InterestRateModel interestRateModel_,
                uint initialExchangeRateMantissa_,
    ...
    function evilSeize(BRToken treasure, address liquidator, address borrower, uint seizeTokens) public returns (uint) {
        return treasure.seize(liquidator, borrower, seizeTokens);
    }
}

contract BRErc20DelegatorScenario is BRErc20Delegator {
    constructor(address underlying_,
                ComptrollerInterface comptroller_,
                InterestRateModel interestRateModel_,
                uint initialExchangeRateMantissa_,
...
contract BRErc20DelegateHarness is BRErc20Delegate {
...
contract BRErc20DelegateScenario is BRErc20Delegate {
    constructor() public {}
...
contract BRErc20DelegateScenarioExtra is BRErc20DelegateScenario {
