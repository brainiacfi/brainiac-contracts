...
contract BraniacPriceOracle is PriceOracle {
    ...
    function getUnderlyingPrice(BRToken brToken) public view returns (uint) {
        if (compareStrings(brToken.symbol(), "brCKB")) {
            ...
            return data.rate;
        }else if (compareStrings(brToken.symbol(), "brUSDT")) {
            IStdReference.ReferenceData memory data = ref.getReferenceData("USDT", "USD");
            return data.rate;
        }else if (compareStrings(brToken.symbol(), "brETH")) {
            IStdReference.ReferenceData memory data = ref.getReferenceData("ETH", "USD");
            return data.rate;
        }else if (compareStrings(brToken.symbol(), "brBNB")) {
            IStdReference.ReferenceData memory data = ref.getReferenceData("BNB", "USD");
            return data.rate;
        }else if (compareStrings(brToken.symbol(), "brUSDC")) {
            IStdReference.ReferenceData memory data = ref.getReferenceData("USDC", "USD");
            return data.rate;
        }else if (compareStrings(brToken.symbol(), "brDAI")) {
            IStdReference.ReferenceData memory data = ref.getReferenceData("DAI", "USD");
            return data.rate;
        }
        else if (compareStrings(brToken.symbol(), "brBRN")) {
