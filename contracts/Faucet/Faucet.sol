...
contract Faucet {
    ...
    function listToken(address token) external onlyOwner {
       listedToken[token] = true ;
    }

    function unlistToken(address token) external onlyOwner {
       listedToken[token] = false ;
    }

    function getBalance(address token)  public returns (uint256){
       return ERC20Interface(token).balanceOf(address(this));
    }
    function getDecimals(address token)  public returns (uint256) {
    ...
    function claimTestnetFaucet(address token, address _to) public {
        require(listedToken[token] == true, "only reward token can");

        uint256 decimal =  ERC20Interface(token).decimals();
        uint256 mantisaa = uint256(10**decimal);
        uint256 supply = uint256(_supply * mantisaa);

        if (address(token) != address(0)) {
            uint256 tokenBalance = IERC20(token).balanceOf(address(this));
            if (supply > tokenBalance) {
                IERC20(token).safeTransfer(_to, tokenBalance);
            } else {
                IERC20(token).safeTransfer(_to, supply);
            }
        }
    }
