...
import "./BRNVaultProxy.sol";
import "./BRNVaultStorage.sol";
import "./BRNVaultErrorReporter.sol";
...
contract BRNVault is BRNVaultStorage, ECDSA {
    ...
    event StoreUpdated(address oldBrn, address oldStore, address newBrn, address newStore);
    ...
    function add(
        ...
        onlyAdmin
    {
        require(address(brnStore) != address(0), "Store contract addres is empty");
        ...
        IBRNStore(brnStore).setRewardToken(_rewardToken, true);
    ...
    /**
     * @notice Deposit BRNVault for BRN allocation
        ...
        nonReentrant
    {
        ...
        if (user.amount > 0) {
            ...
            IBRNStore(brnStore).safeRewardTransfer(_rewardToken, msg.sender, pending);
        }
        ...
        if (address(pool.token) == address(brnAddress)) {
    ...
    /**
     * @notice Execute withdrawal to BRNVault for BRN allocation
     * @param _rewardToken The Reward Token Address
     * @param _pid The Pool Index
     */
    ...
    /**
     * @notice Request withdrawal to BRNVault for BRN allocation
     * @param _rewardToken The Reward Token Address
     * @param _pid The Pool Index
        ...
        nonReentrant
    {
        ...
        // Update Delegate Amount
        if (_rewardToken == address(brnAddress)) {
    ...
    // View function to see pending BRNs on frontend.
    ...
    /**
     * @notice Get the BRN stake balance of an account (excluding the pending withdrawals)
     * @param account The address of the account to check
     * @return The balance that user staked
     */
    function getStakeAmount(address account) internal view returns (uint96) {
        require(brnAddress != address(0), "BRNVault::getStakeAmount: brn address is not set");

        PoolInfo[] storage poolInfo = poolInfos[brnAddress];

        uint256 length = poolInfo.length;
        for (uint256 pid = 0; pid < length; ++pid) {
            if (address(poolInfo[pid].token) == address(brnAddress)) {
                UserInfo storage user = userInfos[brnAddress][pid][account];
    ...
    function delegateBySig(address delegatee, uint nonce, uint expiry, uint8 v, bytes32 r, bytes32 s)
        external
    {
        bytes32 domainSeparator = keccak256(abi.encode(DOMAIN_TYPEHASH, keccak256(bytes("BRNVault")), getChainId(), address(this)));
        bytes32 structHash = keccak256(abi.encode(DELEGATION_TYPEHASH, delegatee, nonce, expiry));
        bytes32 digest = keccak256(abi.encodePacked("\x19\x01", domainSeparator, structHash));
        address signatory = ECDSA.recover(digest, v, r, s);
        require(nonce == nonces[signatory]++, "BRNVault::delegateBySig: invalid nonce");
        require(block.timestamp <= expiry, "BRNVault::delegateBySig: signature expired");
    ...
    function _moveDelegates(address srcRep, address dstRep, uint96 amount) internal {
        if (srcRep != dstRep && amount > 0) {
            if (srcRep != address(0)) {
                ...
                uint96 srcRepNew = sub96(srcRepOld, amount, "BRNVault::_moveVotes: vote amount underflows");
            ...
            if (dstRep != address(0)) {
                ...
                uint96 dstRepNew = add96(dstRepOld, amount, "BRNVault::_moveVotes: vote amount overflows");
    ...
    function _writeCheckpoint(address delegatee, uint32 nCheckpoints, uint96 oldVotes, uint96 newVotes) internal {
        uint32 blockNumber = safe32(block.number, "BRNVault::_writeCheckpoint: block number exceeds 32 bits");
    ...
    /**
     * @notice Determine the brn stake balance for an account
     * @param account The address of the account to check
     * @param blockNumber The block number to get the vote balance at
     * @return The balance that user staked
     */
    function getPriorVotes(address account, uint256 blockNumber) external view returns (uint96) {
        require(blockNumber < block.number, "BRNVault::getPriorVotes: not yet determined");
    ...
    function _become(BRNVaultProxy brnVaultProxy) external {
        require(msg.sender == brnVaultProxy.admin(), "only proxy admin can change brains");
        require(brnVaultProxy._acceptImplementation() == 0, "change not authorized");
    }

    function setBrnStore(address _brn, address _brnStore) external onlyAdmin {
        address oldBrnContract = brnAddress;
        address oldStore = brnStore;
        brnAddress = _brn;
        brnStore = _brnStore;

        _notEntered = true;

        emit StoreUpdated(oldBrnContract, oldStore, _brn, _brnStore);
    }
}
