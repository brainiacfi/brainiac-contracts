...
async function preMintBehalf(brToken, payer, mintAmount, mintTokens, exchangeRate) {
  await preApprove(brToken, payer, mintAmount);
  await send(brToken.comptroller, 'setMintAllowed', [true]);
  await send(brToken.comptroller, 'setMintVerify', [true]);
  await send(brToken.interestRateModel, 'setFailBorrowRate', [false]);
  await send(brToken.underlying, 'harnessSetFailTransferFromAddress', [payer, false]);
  await send(brToken, 'harnessSetBalance', [payer, 0]);
  await send(brToken, 'harnessSetExchangeRate', [ckbMantissa(exchangeRate)]);
}

async function mintBehalfFresh(brToken, payer, receiver, mintAmount) {
  return send(brToken, 'harnessMintBehalfFresh', [payer, receiver, mintAmount]);
}
...
describe('BRToken', function () {
  ...
  beforeEach(async () => {
    ...
    payer = minter;
    brToken = await makeBRToken({comptrollerOpts: {kind: 'bool'}, exchangeRate});
  });
  ...
  describe('mintFresh', () => {
    ...
    it("transfers the underlying cash from payer, tokens to receiver, and emits MintBehalf, Transfer events", async () => {
      const payerBeforeBalances = await getBalances([brToken], [payer]);
      const receiverBeforeBalances = await getBalances([brToken], [receiver]);
      const result = await mintBehalfFresh(brToken, payer, receiver, mintAmount);
      const payerAfterBalances = await getBalances([brToken], [payer]);
      const receiverAfterBalances = await getBalances([brToken], [receiver]);
      expect(result).toSucceed();
      ...
      expect(payerAfterBalances).toEqual(await adjustBalances(payerBeforeBalances, [
        [brToken, payer, 'cash', -mintAmount],
        [brToken, payer, 'tokens', 0],
        [brToken, 'cash', mintAmount],
        [brToken, 'tokens', mintTokens]
      ]));
      expect(receiverAfterBalances).toEqual(await adjustBalances(receiverBeforeBalances, [
        [brToken, receiver, 'cash', 0],
        [brToken, receiver, 'tokens', mintTokens],
        [brToken, 'cash', mintAmount],
        [brToken, 'tokens', mintTokens]
      ]));
    });
  ...
  [redeemFreshTokens, redeemFreshAmount].forEach((redeemFresh) => {
    describe(redeemFresh.name, () => {
      ...
      it("transfers the underlying cash, tokens, and emits Redeem, Transfer events", async () => {
        ...
        expect(afterBalances).toEqual(await adjustBalances(beforeBalances, [
          [brToken, redeemer, 'cash', redeemAmount],
  ...
  describe('redeem', () => {
    ...
    it("returns success from redeemFresh and redeems the right amount", async () => {
      ...
      expect(await balanceOf(brToken.underlying, redeemer)).toEqualNumber(redeemAmount);
    });
