...
describe.skip('BrainiacWheelFuzzTest', () => {
  // This whole test is fake, but we're testing to see if our equations match reality.

  // First, we're going to build a simple simulator of the Brainiac protocol
  ...
  let initialState = globals => {
    return {
      ...
      brainiacSupplySpeed: new bn(1),
      brainiacSupplyIndex: new bn(1),
      brainiacSupplyIndexSnapshots: {},
      brainiacSupplyIndexUpdatedBlock: globals.blockNumber,

      brainiacBorrowSpeed: new bn(1),
      brainiacBorrowIndex: new bn(1),
      brainiacBorrowIndexSnapshots: {},
      brainiacSupplyIndexUpdatedBlock: globals.blockNumber,

      brainiacAccruedWithCrank: {}, // naive method, accruing all accounts every block
      brainiacAccruedWithIndex: {}, // with indices

      activeBorrowBlocks: new bn(0), // # blocks with an active borrow, for which we expect to see brn distributed. just for fuzz testing.
  ...
  let trueUpBrainiac = (globals, state) => {
    state = accrueInterest(globals, state);

    state = Object.keys(state.brainiacSupplyIndexSnapshots).reduce(
      (acc, account) => supplierFlywheelByIndex(globals, state, account),
      state
    );

    state = Object.keys(state.brainiacBorrowIndexSnapshots).reduce(
  ...
  // manual flywheel loops through every account and updates brn accrued mapping
  ...
  let flywheelByCrank = (
    ...
    for (let [account, balance] of Object.entries(balances)) {
      if (isPositive(totalSupply)) {
        brainiacAccruedWithCrank[account] = get(
          state.brainiacAccruedWithCrank[account]
        ).plus(
          deltaBlocks
            .times(brainiacSupplySpeed)
            .times(balance)
            .div(totalSupply)
        );
      }
    ...
    return {
      ...state,
      brainiacAccruedWithCrank: brainiacAccruedWithCrank,
    };
  };

  // real deal brn index flywheel™️
  let borrowerFlywheelByIndex = (globals, state, account) => {
    let {
      brainiacBorrowSpeed,
      brainiacBorrowIndex,
      brainiacBorrowIndexSnapshots,
      brainiacAccruedWithIndex,
      totalBorrows,
      borrowBalances,
      brainiacBorrowIndexUpdatedBlock,
      borrowIndex,
      borrowIndexSnapshots
    } = state;

    let deltaBlocks = globals.blockNumber.minus(brainiacBorrowIndexUpdatedBlock);
    if (isPositive(totalBorrows)) {
      let scaledTotalBorrows = totalBorrows.div(borrowIndex);
      brainiacBorrowIndex = brainiacBorrowIndex.plus(
        brainiacBorrowSpeed.times(deltaBlocks).div(scaledTotalBorrows)
      );
    }

    let indexSnapshot = brainiacBorrowIndexSnapshots[account];

    if (
      indexSnapshot !== undefined &&
      brainiacBorrowIndex.isGreaterThan(indexSnapshot) &&
      ...
      brainiacAccruedWithIndex[account] = get(brainiacAccruedWithIndex[account]).plus(
        borrowBalanceNew
          .div(borrowIndex)
          .times(brainiacBorrowIndex.minus(indexSnapshot))
      );
    }

    return {
      ...state,
      brainiacBorrowIndexUpdatedBlock: globals.blockNumber,
      brainiacBorrowIndex: brainiacBorrowIndex,
      brainiacBorrowIndexSnapshots: {
        ...state.brainiacBorrowIndexSnapshots,
        [account]: brainiacBorrowIndex
      }
    };
  };
  ...
  let supplierFlywheelByIndex = (globals, state, account) => {
    let {
      ...
      brainiacAccruedWithIndex,
      totalSupply,
      brainiacSupplyIndexUpdatedBlock
    } = state;

    let deltaBlocks = globals.blockNumber.minus(brainiacSupplyIndexUpdatedBlock);

    if (isPositive(totalSupply)) {
      brainiacSupplyIndex = brainiacSupplyIndex.plus(
        brainiacSupplySpeed.times(deltaBlocks).div(totalSupply)
      );
    }

    let indexSnapshot = brainiacSupplyIndexSnapshots[account];
    if (indexSnapshot !== undefined) {
      // if had prev snapshot,  accrue some brn
      brainiacAccruedWithIndex[account] = get(brainiacAccruedWithIndex[account]).plus(
        balances[account].times(brainiacSupplyIndex.minus(indexSnapshot))
      );
    }
    ...
    return {
      ...state,
      brainiacSupplyIndexUpdatedBlock: globals.blockNumber,
      brainiacSupplyIndex: brainiacSupplyIndex,
      brainiacSupplyIndexSnapshots: {
        ...state.brainiacSupplyIndexSnapshots,
        [account]: brainiacSupplyIndex
      },
      brainiacAccruedWithIndex: brainiacAccruedWithIndex
    };
  };
  ...
  // assert amount distributed by the crank is expected, that it equals # blocks with a supply * brn speed
  let crankCorrectnessInvariant = (globals, state, events, invariantFn) => {
    let expected = state.activeSupplyBlocks
      .times(state.brainiacSupplySpeed)
      .plus(state.activeBorrowBlocks.times(state.brainiacBorrowSpeed));

    let actual = Object.values(state.brainiacAccruedWithCrank).reduce(
    ...
    invariantFn(
      ...
      expected,
      actual,
      `crank method distributed brn inaccurately`
    );
  };

  // assert brn distributed by index is the same as amount distributed by crank
  let indexCorrectnessInvariant = (globals, state, events, invariantFn) => {
    let expected = state.brainiacAccruedWithCrank;
    let actual = state.brainiacAccruedWithIndex;
    invariantFn(
      (expected, actual) => {
  ...
  let runEvents = (globals, initState, events) => {
    ...
    return trueUpBrainiac(globals, state);
  };
